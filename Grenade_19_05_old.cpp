...
STATE_SWITCH_ON					// Помигать, попищать и спать
STATE_SLEEP						// во сне
STATE_WAKE_UP					// проснулись, помигать, попищать
STATE_RESPAWN					// ожидаем IR сигнала на оживление
STATE_TRIGGER					// во сне, ожидаем нажатия на кнопку
STATE_EXPLODE					// взрыв, свет, звук
STATE_FIND_ME					// мигаем пищим, ожидаем RESP-IR или короткого нажатия или длиного
...
static int protothreadWaitForTrigger(struct pt *pt) {
  PT_BEGIN(pt);
  while(1) {
	PT_WAIT_UNTIL(pt, (state != STATE_STARTED) || (state != STATE_ARMED));
	sleep_enable();
	sei();
	sleep_cpu();
	sleep_disable();
